...
- **Truth drift.** The code moved on and the document no longer describes it. No file comparison settles this. `documentation` owns DESIGN.md writing; `document` extracts the visual evidence; `init` owns PRODUCT.md. This command's job is to hand them a specific gap rather than a vague suspicion.
