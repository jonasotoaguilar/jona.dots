 ...
 *   isGitIgnored(filePath, projectCwd)
 *   loadDetector() -> Promise<{ detectText, detectHtml }>
 *   matchesAnyGlob(filePath, globs)
 *   normalizeScanTargets(primaryTargets, projectCwd)
 *   findingCacheKey(finding)
 *   runHook(deps) -> { exitCode, stdout, audit, reason? }
 *   runStopHook(deps) -> { exitCode, stdout, audit, emission? }
 *   runSnapshot(deps) -> side-effect-free full scan (before/after legs)
 *   snapshotTargets(targetFiles, projectCwd, opts) -> fixed-set after leg
 *   diffSnapshotDelta(baseline, after) -> new-findings-only groups
 *   renderSnapshotFindings(groups, render) -> additionalContext envelope
 *
 * Design notes:
...
// Git is the source of truth for repository ignore rules. `--no-index` keeps
// the hook from scanning tracked files that still match an ignore pattern and
// also catches newly-created ignored files before they are added to the index.
// A non-Git directory, missing Git, or a timed-out check fails open so the
// design hook remains advisory rather than becoming a repository prerequisite.
export function isGitIgnored(filePath, projectCwd) {
  if (!filePath || !projectCwd || hasPathTraversal(filePath)) return false;

  let relativePath;
  try {
    relativePath = path.relative(projectCwd, filePath);
  } catch {
    return false;
  }
...
// Sync the immediate tier while retaining deferred keys remembered by a prior
// deep pass. That keeps first-time deferred findings fresh for the next deep
// pass without re-emitting unchanged deferred findings after every later edit.
function rememberImmediateFindings(cache, sessionId, filePath, findings) {
  const fileEntry = ensureFile(cache, sessionId, filePath);
  const deferredKeys = (fileEntry.findings || []).filter((key) => {
    const rule = normalizeIgnoreRule(String(key).split(':', 1)[0]);
    return !IMMEDIATE_TIER_RULES.has(rule);
  });
  const immediateKeys = (findings || []).map(f => findingCacheKey(f));
  fileEntry.findings = Array.from(new Set([...deferredKeys, ...immediateKeys]));
  ensureSession(cache, sessionId).updatedAt = Date.now();
}

// Stable finding signature used by both the session cache and the snapshot
// delta. Prefers rule value, then snippet, then line: value and snippet stay
// stable under line movement, so a finding that merely shifts lines must not
// read as new.
export function findingCacheKey(finding) {
  const value = extractFindingIgnoreValue(finding);
  if (value) return `${finding.antipattern}:0:${value}`;
  const snippet = String(finding?.snippet || '').trim().slice(0, 80);
  if (snippet) return `${finding.antipattern}:0:${snippet}`;
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    for (const filePath of targetFiles) {
      ...
      // Sync the immediate tier before deciding what to emit, so fixed
      // immediate findings disappear and reintroductions read as fresh. Keep
      // deferred keys already reported by a prior deep pass; new deferred
      // findings still remain absent and therefore fresh to the next one.
      if (tiered) {
        rememberImmediateFindings(cache, sessionId, filePath, immediate);
      } else {
...
export async function runStopHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    for (const filePath of touched) {
      ...
      if (isGitIgnored(filePath, projectCwd)) continue;
      const ext = path.extname(filePath).toLowerCase();
      const configuredExt = matchConfiguredExtension(filePath, config.extensions);
      ...
      // Sync the cache to the complete current scan, not just the fresh
      // findings (see rememberFindings' contract). Fixing this is what stops
      // consecutive Stop passes from alternating: with only the fresh set
      // remembered, a finding deduped this run is forgotten and reads fresh
      // again next run. Remembering the full filtered set also lets a clean
      // scan shrink memory so a fixed finding stops being reported and a
      // reintroduced one fires again.
    ...
    // Persist the corrected cache even when the scan is clean: fixed findings
    // must disappear from memory and a reintroduced finding must fire once.
    if (cacheDirty) {
      persistCache(projectCwd, cache);
    }
...
// ────────────────────────────────────────────────────────────────────────
// Side-effect-free snapshot + delta API (OpenCode plugin path).
//
// `runSnapshot` (before an edit) and `snapshotTargets` (after, over the exact
// target list the before pass stored) run a FULL detector pass with the SAME
// detector, config/ignore rules, safety gates and path resolution as the
// per-edit hook — but with no session-cache mutation and no emitted
// messages. `diffSnapshotDelta` then attributes only what the edit actually
// introduced: it diffs by stable signature with multiplicity, so untouched
// findings never re-report, a second identical occurrence reads as exactly
// one new finding, and line movement is not a new finding. `renderSnapshotFindings`
// rebuilds the original additionalContext envelope from a delta result.
// ────────────────────────────────────────────────────────────────────────

async function snapshotScanFiles(targetFiles, projectCwd, config, det, scanOptions) {
  const files = {};
  const groups = [];
  for (const filePath of targetFiles) {
    const skippedState = (skipReason) => {
      files[filePath] = { status: 'skipped', skipReason, findings: [] };
    };

    if (hasPathTraversal(filePath) || SENSITIVE_PATH.test(filePath)) { skippedState('sensitive'); continue; }
    if (GENERATED_PATH.test(filePath)) { skippedState('generated'); continue; }
    if (isGitIgnored(filePath, projectCwd)) { skippedState('git-ignore'); continue; }

    const ext = path.extname(filePath).toLowerCase();
    const configuredExt = matchConfiguredExtension(filePath, config.extensions);
    if (!ALLOWED_EXTS.has(ext) && !configuredExt) { skippedState('extension'); continue; }

    const relForMatch = relativize(filePath, projectCwd);
    if (matchesAnyGlob(relForMatch, config.ignoreFiles) || matchesAnyGlob(filePath, config.ignoreFiles)) {
      skippedState('config-ignore-file');
      continue;
    }

    // A target that does not exist yet is a legitimate empty baseline: the
    // edit may create it (write / apply_patch Add File). Everything else that
    // blocks the scan is recorded so the delta can fail closed.
    if (!fs.existsSync(filePath)) { files[filePath] = { status: 'missing', findings: [] }; continue; }
    if (!isScanTargetInsideProject(filePath, projectCwd)) { skippedState('outside-project'); continue; }

    const maxFileBytes = config.limits?.maxFileBytes ?? DEFAULT_CONFIG.limits.maxFileBytes;
    if (maxFileBytes > 0) {
      let size = 0;
      try { size = fs.statSync(filePath).size; } catch { size = 0; }
      if (size > maxFileBytes) { skippedState('too-large'); continue; }
    }

    let content;
    try { content = fs.readFileSync(filePath, 'utf-8'); } catch { skippedState('unreadable'); continue; }
    ...
    if (detectorThrew) { skippedState('detector-threw'); continue; }

    const filtered = filterFindings(findings || [], content, ext, config);
    files[filePath] = { status: 'scanned', findings: filtered };
    if (filtered.length > 0) {
      groups.push({ filePath, findings: filtered });
    }
  }
  return { files, groups };
}
...
/**
 * Full side-effect-free snapshot over the targets an operation is about to
 * affect (or has affected). Same result contract shape for the before and
 * after legs:
 *   { ok: true, projectCwd, sessionId, targetFiles, files, groups, render, audit }
 *   { ok: false, skipped, audit }
 * Never throws; never mutates the session cache; never emits messages.
 */
export async function runSnapshot({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  const started = Date.now();
  const audit = { ts: new Date(now()).toISOString(), event: 'OpenCodeSnapshot' };
  const fail = (skipped) => ({ ok: false, skipped, audit: { ...audit, skipped, durationMs: Date.now() - started } });

  if (depthIsSet(env.IMPECCABLE_HOOK_DEPTH) || depthIsSet(env.CLAUDE_HOOK_DEPTH)) {
    return fail('reentrant');
  }
  if (truthy(env.IMPECCABLE_HOOK_DISABLED)) {
    return fail('env-disabled');
  }
  ...
  try {
    ...
    return fail('stdin-malformed');
  }
  if (!event || typeof event !== 'object') {
    return fail('stdin-empty');
  }
  ...
  const config = readConfig(projectCwd);
  if (config.enabled === false) {
    return fail('config-disabled');
  }

  const platform = resolveProjectPlatform(projectCwd);
  if (isNativePlatform(platform)) {
    return fail('native-platform');
  }

  const det = detector || await loadDetector();
  if (!det || typeof det.detectText !== 'function') {
    return fail('detector-missing');
  }

  const scanOptions = designSystemOptions(config, det, projectCwd);
  const { files, groups } = await snapshotScanFiles(targetFiles, projectCwd, config, det, scanOptions);
  ...
  return {
    ...
    files,
    groups,
    render: { config, scanOptions, projectCwd },
    audit: {
      ...audit,
      scannedFiles: Object.keys(files).length,
      findingFiles: groups.length,
      freshFindings: groups.reduce((sum, group) => sum + group.findings.length, 0),
      durationMs: Date.now() - started,
    },
  };
}
...
/**
 * Snapshot over a FIXED target list and project root — the after leg of the
 * OpenCode plugin's before/after pair, scanning exactly the targets the
 * before pass stored so both sides are comparable.
 */
export async function snapshotTargets(targetFiles, projectCwd, { sessionId = 'unknown', env = {}, now = Date.now, detector } = {}) {
  const started = Date.now();
  const audit = { ts: new Date(now()).toISOString(), event: 'OpenCodeSnapshot' };
  const fail = (skipped) => ({ ok: false, skipped, audit: { ...audit, skipped, durationMs: Date.now() - started } });
  ...
  const project = path.resolve(projectCwd || process.cwd());
  ...
  const platform = resolveProjectPlatform(project);
  if (isNativePlatform(platform)) {
    return fail('native-platform');
  }
  ...
  const scanOptions = designSystemOptions(config, det, project);
  const { files, groups } = await snapshotScanFiles(targetFiles, project, config, det, scanOptions);
  ...
  return {
    ...
    files,
    groups,
    render: { config, scanOptions, projectCwd: project },
    audit: {
      ...audit,
      cwd: project,
      session: sessionId,
...
// Multiplicity diff: a signature present N times before and M times after
// yields at most M - N new findings (first instances in scan order). A second
// identical occurrence of an existing finding is exactly one new finding;
// an unchanged occurrence contributes zero.
function multiplicityDiff(baselineFindings, afterFindings) {
  const baselineCounts = countSignatures(baselineFindings);
  const afterCounts = countSignatures(afterFindings);
  const fresh = [];
  for (const [key, afterCount] of afterCounts) {
    const delta = afterCount - (baselineCounts.get(key) || 0);
    if (delta <= 0) continue;
    const instances = afterFindings.filter((f) => findingCacheKey(f) === key);
    fresh.push(...instances.slice(0, delta));
  }
  return fresh;
}

/**
 * Diff a stored before-snapshot against a fresh after-snapshot. Returns the
 * groups of findings the operation actually introduced. Per-file fail-closed
 * gating: findings from a target whose baseline was unreadable or skipped are
 * never attributed to the edit, because the baseline could not prove they
 * were absent before. A target whose baseline was `missing` (a new file) has
 * an empty baseline, so all of its after findings are attributable.
 */
export function diffSnapshotDelta(baseline, after) {
  if (!baseline || !after || baseline.ok !== true || after.ok !== true) return [];
  const groups = [];
  for (const [filePath, afterState] of Object.entries(after.files || {})) {
    if (!afterState || afterState.status !== 'scanned') continue;
    if (!Array.isArray(afterState.findings) || afterState.findings.length === 0) continue;
    const baseState = baseline.files?.[filePath];
    if (!baseState) continue;
    if (baseState.status === 'missing') {
      groups.push({ filePath, findings: afterState.findings });
      continue;
    }
    if (baseState.status !== 'scanned') continue;
    const fresh = multiplicityDiff(baseState.findings || [], afterState.findings);
    if (fresh.length > 0) {
      groups.push({ filePath, findings: fresh });
    }
  }
  return groups;
}
...
/**
 * Render delta groups in the original Impeccable guidance format (the same
 * envelope `runHook`/`runStopHook` emit as additionalContext).
 */
export function renderSnapshotFindings(groups, render = null) {
  if (!Array.isArray(groups) || groups.length === 0) return '';
  const config = render?.config || DEFAULT_CONFIG;
  const text = renderGroupedTemplate(groups, config, { cwd: render?.projectCwd });
  return text ? appendDesignSystemNote(text, render?.scanOptions) : '';
}
