...
Many pipelines trigger on a version tag and publish directly, without semantic-release. No semantic-release configuration is needed, but one rule always holds: `npm publish` publishes the version in `package.json`, not the tag's version. A tag `v2.0.0` with a `package.json` still at `1.5.0` publishes `1.5.0`. Sync the version inside the workflow before publishing:
...
Release-workflow design (tag trigger, go/no-go, publication order) is owned by `ci-cd-and-automation`/`shipping-and-launch`; this section covers the npm publishing-domain rules only.

## semantic-release (optional — only when release CI uses it)

semantic-release is one release mechanism, not the default. Tag-triggered workflows (above) need no semantic-release configuration; apply this section only when the project's release pipeline actually uses it.
