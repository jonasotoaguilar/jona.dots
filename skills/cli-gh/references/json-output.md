...
gh repo view owner/repo --json name,description,stargazerCount,forkCount,updatedAt,url,defaultBranchRef
```

`readme` is not a valid `gh repo view --json` field (verified against gh 2.97.0). Fetch README content via
`gh api repos/:owner/:repo/readme` instead.
