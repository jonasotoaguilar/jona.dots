...
> Syntax/automation patterns only. Execution routes through the owning skills: `branch-pr` (PR create/update), `merge-pr` (PR merge), `comment-writer` (public comments/reviews), `issue-creation` (issue publication), `systemic-issue-triage` (bug triage/backlog).
...
Release creation is NOT autonomous here: no git pull / version bump / tag / push / generated-notes pipeline. Manual release creation happens only when explicitly authorized by `shipping-and-launch` and must consume its curated notes contract (`--notes-file docs/releases/<tag>.md`); workflow-driven release mechanics belong to `ci-cd-and-automation`. See `releases.md` for the boundary and the authorized command forms.
