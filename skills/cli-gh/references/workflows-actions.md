...
Trigger workflows with custom inputs. Since v2.87, `gh workflow run` prints the workflow run URL when available; when it is not, verify via the returned run ID with `gh run view`.
...
# Trigger workflow - the run URL is printed when available
...
# Trigger and watch in one go (resolve the run ID, do not parse the printed URL)
gh workflow run ci.yml --ref main
RUN_ID=$(gh run list --workflow=ci.yml --limit 1 --json databaseId --jq '.[0].databaseId')
gh run view "$RUN_ID"
```

> Syntax reference only. Merging PRs executes through the `merge-pr` skill; PR comments and reviews route through
> `comment-writer` / `branch-pr`.
...
gh run list --created=">=$(date -d '30 days ago' +%Y-%m-%d)" --status=failure --json id | jq '. | length'
...
> `gh secret set` is a write: run only on explicit user request and never print secret values in output. Prefer
> passing the value from a file or stdin so it does not appear in shell history.
```
