# Animation Standards & Audit Playbook

The precise values, curves, and rules behind motion review, audit, and improvement. Cite these in findings instead of approximating; never approximate a value that appears here — copy it. Distilled from Emil Kowalski's design engineering philosophy (emilkowal.ski, animations.dev).

Sections carry a **Hunt for** checklist for audit mode (improve) and citation values for review mode. One source serves both: `ui-motion review` cites; `ui-motion improve` audits.
...
- **Hunt for**: animations on keyboard-initiated actions, command palettes with open/close transitions, decorative motion on list items or hover states hit constantly. The strongest fix is often **delete the animation**.
...
- **Hunt for**: `ease-in` anywhere, bare `ease`/`linear` on entrances, tooltip delay + animation on every tooltip in a toolbar (after the first, they should be instant).
...
**Complex transitions: ≤400ms normally**, up to 500ms only as a documented exception (e.g. a deliberate overlay/hero moment). Anything slower on a UI element needs justification or it's a finding.

- **Hunt for**: durations > 300ms on UI elements; complex transitions above 400ms without a documented exception.
...
- **Origin-aware popovers.** Scale from the trigger, not center:
  ...
  ```
  **Modals are exempt** — they appear centered in the viewport, keep `transform-origin: center`. Do not report it.
...
Deliberate phases (press, hold, destructive confirm) animate slower; the system's response snaps. Symmetric timing on press-and-release is a finding.
...
Stagger group entrances; **30–50ms between items**. Longer delays feel slow. Stagger is decorative — never block interaction while it plays.
...
- **Hunt for**: everything-at-once group entrances where a 30–50ms stagger belongs; duplicated near-identical easings/durations (token consolidation finding).
...
Curves and durations should live as shared tokens. Five hand-typed cubic-beziers that almost match is a consolidation finding. A jarring crossfade that shows two overlapping states can be masked with subtle `filter: blur(2px)`.

- **Hunt for**: one bouncy component in a crisp app, list/grid entrances with no stagger, crossfades that visibly double-expose.
