...
A search skill. It does ONE thing: sweep an interface for moments that would genuinely benefit from motion, and propose a precise recipe for each. It does not review existing animations (that's `ui-motion review`), audit and plan fixes for them (that's `ui-motion improve`), or write the implementation itself.
...
1. **Never modify source code.** This skill reports; it does not implement. If asked to build a suggestion, hand it off (e.g. `ui-motion improve plan <description>`, or let the user take the recipe to any agent).
...
Use the frequency table in `standards.md` (Should it animate?): 100+/day → **Reject. No animation. Ever.**; tens/day → reject or near-imperceptible motion only; occasional → eligible, standard animation; rare/first-time → eligible, the delight budget lives here. Keyboard-initiated actions are a disqualifier, not a judgment call — repeated hundreds of times a day, animation makes them feel slow, delayed, and disconnected. Raycast has no open/close animation; that is the optimal experience.
...
The suggestion must work within the standard budgets in `standards.md` (UI under 300ms; per-element duration table). If the moment only "works" as a slow, showy animation, it fails the gate.
...
One short paragraph: how much motion this interface actually needs, whether it's already close to right, and which single suggestion has the highest leverage. Close by pointing at the handoff: `ui-motion improve plan <suggestion>` to turn any row into a self-contained implementation plan.
